 ...
 * Handles rendering individual phase content for job descriptions
...
export function renderPhaseContent(project, phaseNumber) {
  ...
  if (phaseNumber === 3 && phaseData.completed) {
    completionBanner = renderCompletionBanner(phaseData.response || '', project);
  }
 ...
 * @returns {string} HTML string
 */
function renderCompletionBanner(content, project) {
  const validationResult = validateDocument(content, project.postingType || 'external');
