...
export class Workflow {
  ...
  async generatePrompt() {
    ...
    const formData = {
      ...
      compensationRange: p.compensationRange,
      benefits: p.benefits,
      techStack: p.techStack,
      teamSize: p.teamSize,
      aiSpecifics: p.aiSpecifics,
      careerLadder: p.careerLadder,
      companyPreamble: p.companyPreamble,
      companyLegalText: p.companyLegalText
    };
  ...
  replaceVariables(template) {
    ...
    // Role Basics - required fields get specific labels
    result = result.replace(/\{jobTitle\}/g, val(p.jobTitle, 'Job title'));
    result = result.replace(/\{companyName\}/g, val(p.companyName, 'Company name'));
    result = result.replace(/\{roleLevel\}/g, optVal(p.roleLevel));
    result = result.replace(/\{location\}/g, optVal(p.location));

    // Responsibilities & Requirements
    result = result.replace(/\{responsibilities\}/g, optVal(p.responsibilities));
    result = result.replace(/\{requiredQualifications\}/g, optVal(p.requiredQualifications));
    result = result.replace(/\{preferredQualifications\}/g, optVal(p.preferredQualifications));

    // Compensation & Benefits
    result = result.replace(/\{compensationRange\}/g, optVal(p.compensationRange));
    result = result.replace(/\{benefits\}/g, optVal(p.benefits));

    // Optional Context
    result = result.replace(/\{techStack\}/g, optVal(p.techStack));
    result = result.replace(/\{teamSize\}/g, optVal(p.teamSize));
    result = result.replace(/\{aiSpecifics\}/g, optVal(p.aiSpecifics));
    result = result.replace(/\{careerLadder\}/g, optVal(p.careerLadder));

    // Company-Mandated Content
    result = result.replace(/\{companyPreamble\}/g, optVal(p.companyPreamble));
    result = result.replace(/\{companyLegalText\}/g, optVal(p.companyLegalText));
